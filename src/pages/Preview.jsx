...
const Preview = () => {
  ...
  const mapParticipantData = (p) => {
    ...
    return {
      ...
      email: p.email || "",
      address: p.residence || p.address || "",
      fatherName: p.fatherName || "",
      motherName: p.motherName || "",
      contactFatherMobile: p.contactFatherMobile || "",
      contactMotherMobile: p.contactMotherMobile || "",
      parentAgreement: p.parentAgreement || false,
      parentSignature: p.parentSignature || "",
      medicalConditions: medConds,
      additionalMedicalNotes: medConds.includes("N/A") ? "N/A" : (p.additionalMedicalNotes || p.medicalNotes || ""),
      otherCondition: p.otherCondition || "",
  ...
  const handleChange = (index, field, value) => {
    setParticipants((prev) => {
      const updated = [...prev];
      // Map 'residence' field to 'address' for consistency
      if (field === "residence" || field === "address") {
        updated[index]["address"] = value;
        updated[index]["residence"] = value;
      } else {
    ...
    try {
      ...
      for (let p of participants) {
        ...
        const data = {
          ...
          additionalMedicalNotes: (Array.isArray(p.medicalConditions) && p.medicalConditions.includes("N/A")) ? "N/A" : (p.additionalMedicalNotes || p.medicalNotes || ""),
          otherCondition: p.otherCondition || "",
          medicalNotes: p.medicalNotes || "",
          studentId,
          familyId: studentId,
